module.exports = {
    ...
    async execute(Discord, client, args, message, MessageEmbed, profileModel){
        ...
        if (userPinged.bot || message.mentions.roles.first()) return message.author.send("YOU IDIOT THAT WAS A BOT???")
            ...
            let profileDataPinged = profileData = await profileModel.findOne({userID: userPinged.id}); //Attempts to look for a user in the DB with the user's id
            ...
            userProfile
            .setThumnail(userPinged.displayAvatarURL())
            .addFields(
